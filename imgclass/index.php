...
class Image{ //类定义开始
	 ...
	 * @return unknown
	 */
	static function waterMark($uploadFile, $watermarkSize = '100x100', $watermarkPosition = 0, $watermarkPadding = 5, $watermarkTrans = 70, $waterImg) {
		$watermarkSize = empty ( $watermarkSize ) ? '100x100' : $watermarkSize;
		$watermarkPosition = empty ( $watermarkPosition ) ? 0 : $watermarkPosition;
		$watermarkPadding = empty ( $watermarkPadding ) ? 5 : $watermarkPadding;
		$watermarkTrans = empty ( $watermarkTrans ) ? 70 : $watermarkTrans;
		...
		$watermark_size = explode ( 'x', $watermarkSize );
		if ($uploadInfo [0] < $watermark_size [0] || $uploadInfo [1] < $watermark_size [1])
			return false;
		...
		switch ($watermarkPosition) {
			...
			// random
			default :
				$pos_x = rand ( 0, ($uploadInfo [0] - $width) );
				$pos_y = rand ( 0, ($uploadInfo [1] - $height) );
				break;
		}
		//透明度
		if ($imgmark = @imagecreatefrompng ( $waterImg )) {
			...
			if ($watermarkTrans) {
				@imagecopymerge ( $tmp, $imgmark, $pos_x, $pos_y, 0, 0, $width, $height, $watermarkTrans );
			} else {
